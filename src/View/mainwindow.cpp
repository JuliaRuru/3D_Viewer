...
MainWindow::MainWindow(Controller *controller, QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), my_controller_(controller) {
  ui->setupUi(this);
  ...
  connect(ui->standart_apply_button, SIGNAL(clicked()), this,
          SLOT(reset_settings()));
...
void MainWindow::open_file() {
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Открыть файл"), "../../../../s21_3D_Viewer_2_0/src/obj/.", "*.obj");
  if (filename != "") {
    reset_settings();
    ...
    double norm_scale;
    my_controller_->CountElements(buff, &norm_scale);
    my_controller_->FillingMatrix(buff, &norm_scale);
    ui->openGLWidget->picture->filepath = filename;

    ui->openGLWidget->picture->initPicture(my_controller_);

    ui->file_location->setText(filename);
    QString text_vertexes =
        QString::number(my_controller_->getCountOfVertexes());
    ui->count_vertex->setText(text_vertexes);
    QString text_facets = QString::number(my_controller_->getCountOfFacets());
...
void MainWindow::on_move_x_slider_valueChanged(int value) {
  ...
  my_controller_->ModelMove('x', new_val - x_new_move);
  x_new_move = new_val;
  ui->openGLWidget->update();
}
...
void MainWindow::on_move_x_box_valueChanged(double arg1) {
  ...
  my_controller_->ModelMove('x', arg1 - x_new_move);
  x_new_move = arg1;
  ui->openGLWidget->update();
}
...
void MainWindow::on_move_y_slider_valueChanged(int value) {
  ...
  my_controller_->ModelMove('y', new_val - y_new_move);
  y_new_move = new_val;
  ui->openGLWidget->update();
}
...
void MainWindow::on_move_y_box_valueChanged(double arg1) {
  ...
  my_controller_->ModelMove('y', arg1 - y_new_move);
  y_new_move = arg1;
  ui->openGLWidget->update();
}
...
void MainWindow::on_move_z_slider_valueChanged(int value) {
  ...
  my_controller_->ModelMove('z', new_val - z_new_move);
  z_new_move = new_val;
  ui->openGLWidget->update();
}
...
void MainWindow::on_move_z_box_valueChanged(double arg1) {
  ...
  my_controller_->ModelMove('z', arg1 - z_new_move);
  z_new_move = arg1;
  ui->openGLWidget->update();
}
...
void MainWindow::on_rotate_x_slider_valueChanged(int value) {
  ...
  my_controller_->ModelRotate('x', new_val - x_new_rotate);
...
void MainWindow::on_rotate_x_box_valueChanged(double arg1) {
  ...
  my_controller_->ModelRotate('x', arg1 - x_new_rotate);
  x_new_rotate = arg1;
  ui->openGLWidget->update();
}
...
void MainWindow::on_rotate_y_box_valueChanged(double arg1) {
  ...
  my_controller_->ModelRotate('y', arg1 - y_new_rotate);
  y_new_rotate = arg1;
  ui->openGLWidget->update();
}
...
void MainWindow::on_rotate_y_slider_valueChanged(int value) {
  ...
  my_controller_->ModelRotate('y', new_val - y_new_rotate);
...
void MainWindow::on_rotate_z_box_valueChanged(double arg1) {
  ...
  my_controller_->ModelRotate('z', arg1 - z_new_rotate);
  z_new_rotate = arg1;
  ui->openGLWidget->update();
}
...
void MainWindow::on_rotate_z_slider_valueChanged(int value) {
  ...
  my_controller_->ModelRotate('z', new_val - z_new_rotate);
...
void MainWindow::on_scale_slider_valueChanged(int value) {
  ...
  if (value != 0) {
    my_controller_->ModelScale(new_val / scale_new);
    scale_new = new_val;
  }
  ui->openGLWidget->update();
}
...
void MainWindow::on_scale_box_valueChanged(double arg1) {
  ...
  if (arg1 != 0.0) {
    my_controller_->ModelScale(arg1 / scale_new);
    scale_new = arg1;
  }
  ui->openGLWidget->update();
}
...
void MainWindow::export_jpeg() {
  ...
  QString filepath = QFileDialog::getExistingDirectory();
  ui->openGLWidget->grabFramebuffer().save(
      filepath + "/" + curdate + curtime + ".JPG", NULL, 100);
}
...
void MainWindow::timer_for_gif() {
  ...
  *photo = ui->openGLWidget->grabFramebuffer();
  ...
  if (time == 50) {
    timer->stop();
    gif->save(QFileDialog::getExistingDirectory() + "/" + curdate + " " +
              curtime + ".GIF");
    delete timer;
    delete gif;
    delete photo;
...
void MainWindow::save_settings() {
  ...
  settings->setValue("flag_dot_round",
                     ui->openGLWidget->picture->flag_dot_round);
...
void MainWindow::load_settings() {
  ui->openGLWidget->check_projection =
      settings->value("check_projection").toBool();
  if (ui->openGLWidget->check_projection)
  ...
  *ver_color = settings->value("color_ver").toByteArray().constData();
  ...
  ui->openGLWidget->picture->flag_dot_round =
      settings->value("flag_dot_round").toBool();
  ui->openGLWidget->picture->flag_dot_vert =
      settings->value("flag_dot_vert").toBool();
  if (ui->openGLWidget->picture->flag_dot_round)
    ui->round_vertexes->setChecked(1);
  else if (ui->openGLWidget->picture->flag_dot_vert)
  ...
  if (ui->openGLWidget->line_dot)
    ui->dotted_facets->setChecked(1);
  else
    ui->solid_facets->setChecked(1);
  ui->openGLWidget->picture->line_width =
      settings->value("line_width").toDouble();
  ui->facets_width_box->setValue(ui->openGLWidget->picture->line_width);

  ui->openGLWidget->update();
}

void MainWindow::on_reset_settings_clicked() { default_settings(); }

void MainWindow::on_load_settings_clicked() { load_settings(); }
